...
gold_ori = 0.1
cash_percent = 0.4 #每次最多拿出30%现金来进行交易
gold_percent = 0.4
bc_percent = 0.4
yield_ratio = 0.9
sharp_ratio = 0.1
...
def sell_bc(day,day_gold):
    ...
    for a in range(1,11):
        ...
        proper_value_before = proper_temp[0] + proper_temp[1] * proper_temp[3] + proper_temp[2] * df_bc.loc[day, "USD"]
        proper_temp[0] = proper_temp[0] + (1-bc_ratio) * proper_temp[2] * real_bc_percent * a * df_bc.loc[day, "USD"]
        proper_temp[2] = proper_temp[2] * (1 - real_bc_percent * a)
        proper_value_after = proper_temp[0]  + proper_temp[1] * df_gold.loc[day_gold+1, "pred"] + proper_temp[2] * df_bc.loc[day+1, "pred"]
...
#     gold_ratio = 0.01
#     bc_ratio = 0.02
#     gold_ratio = gold_ratio + a/1000
#     bc_ratio = bc_ratio + a/1000
#     proper = [1000,0,0,0]
#     make_deal(Value[a+2],Propers[0])
#     plt.plot(Value[a+2])
#     if(a<2):
#         Date.clear()
# #headers = ['bc_ratio = 0.01', 'bc_ratio = 0.015','bc_ratio = 0.02','bc_ratio = 0.025','bc_ratio = 0.03','Date']
# #headers = ['gold_ratio = 0.002', 'gold_ratio = 0.006','gold_ratio = 0.01','gold_ratio = 0.015','gold_ratio = 0.02','Date']
...
# df.to_csv('ratio_change_new/gold_bc_change.csv')
# plt.show()
